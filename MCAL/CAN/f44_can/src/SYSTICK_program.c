...
void MSysTick_Handler(void)
{
	...
	{
		Global_f32Counter--;
		MSYSTICK_voidSetIntervalSinglee(Global_u32Ticks,*Global_ptr);

	}
}
// ----------------------------------------------------------------------------
void MSYSTICK_voidSetIntervalSinglee( u32 copy_u32Ticks ,void (*Copy_ptr)(void) )
{
	/*if(copy_u8TickType == MILLI_SEC)
	{
		copy_u32Ticks *= 21276;
	...
	*/
	...
	else if(copy_u32Ticks > 16777210)
	{
		Global_f32Counter = copy_u32Ticks/16777216;
		/* Disable the counter */
		CLR_BIT(STK->CTRL, STK_CTRL_ENABLE_BIT);
		STK->VAL =0;
		/* Update the Load registers with number of Ticks or counts */
		...
		//Global_u8TickType=copy_u8TickType;
		Global_ptr= Copy_ptr;
		Global_u32Ticks=copy_u32Ticks-16777210;
		/* Enable the Timer */
		SET_BIT(STK->CTRL, STK_CTRL_ENABLE_BIT);
